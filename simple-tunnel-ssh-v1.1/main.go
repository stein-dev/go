...
import (
	...
	"github.com/stein/simple-tunnel-ssh-v1.1/m/libinject"
	"github.com/stein/simple-tunnel-ssh-v1.1/m/libutils"
	"github.com/stein/simple-tunnel-ssh-v1.1/m/libsshclient"
...
func main() {
	...
	liblog.LogInfo("Simple Tunnel SSH v0.1.1 | @pigscanfly", "", Colors["B1"])

	key := "d5b8f3d1c8adceafbd77fb3c991e4cf78d174dd4b71f988692fc31c46eaa242e"

	if _, err := os.Stat("config/enc-profile.json"); err == nil {
		...
		liblog.LogInfo("Unknown error occured. Please contact NBI.", "", Colors["B1"])
	  }
	...
	if err != nil {
		...
		//fmt.Println(cf.SshSettings.Host)
	...
	//libutils.JsonReadWrite(libutils.RealPath("config/enc-inject.json"), config, defaultConfig)
	...
	// 	LogInfo("No arguments supplied. Exiting", "INFO", Colors["R1"])
	// 	os.Exit(1)
	// }
	...
	LogInfo("==========================================", "", Colors["B1"])
	LogInfo("Note: PH server provided by @Maryrose143", "", Colors["G1"])
	LogInfo("==========================================", "", Colors["B1"])
	...
	if err != nil {
		//log.Println(err)
		//LogInfo(string(err), "INFO", Colors["R1"])
